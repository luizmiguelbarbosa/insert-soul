...
static Image gifData = {0}; // CRÍTICO: Mantém o buffer da animação
static Texture2D staticSprite = {0}; // NOVA: Fallback de imagem estática
static bool hasStatic = false; // NOVA: Flag para fallback
...
static int cmp_raw_event(const void *pa, const void *pb) { const RawEvent *a = (const RawEvent*)pa; const RawEvent *b = (const RawEvent*)pb; if (a->tick < b->tick) return -1; if (a->tick > b->tick) return 1; return 0; }
...
// --- VISUAIS ---
void DrawScanlines(int screenW, int screenH) { for (int y = 0; y < screenH; y += 4) DrawRectangle(0, y, screenW, 1, Fade(BLACK, 0.2f)); }
void DrawVignette(int screenW, int screenH) { DrawRing((Vector2){screenW/2, screenH/2}, screenH*0.5f, screenH*0.9f, 0, 360, 0, Fade(BLACK, 0.0f)); DrawRing((Vector2){screenW/2, screenH/2}, screenH*0.9f, screenH*1.2f, 0, 360, 0, BLACK); }
void SpawnExplosion(Vector2 pos, Color color) { for (int i = 0; i < 20; i++) { particlePoolIndex = (particlePoolIndex + 1) % MAX_PARTICLES; particles[particlePoolIndex] = (Particle){pos, {(float)GetRandomValue(-200, 200), (float)GetRandomValue(-250, 50)}, (GetRandomValue(0, 10) < 5) ? color : WHITE, PARTICLE_LIFE, true}; } }
void SpawnMiss(Vector2 pos) { for (int i = 0; i < 10; i++) { particlePoolIndex = (particlePoolIndex + 1) % MAX_PARTICLES; particles[particlePoolIndex] = (Particle){(Vector2){pos.x + GetRandomValue(-10,10), pos.y}, {(float)GetRandomValue(-50, 50), (float)GetRandomValue(50, 150)}, RED, 0.5f, true}; } }
void SpawnSustainSparks(Vector2 pos, Color color) { for (int i = 0; i < 1; i++) { particlePoolIndex = (particlePoolIndex + 1) % MAX_PARTICLES; particles[particlePoolIndex] = (Particle){(Vector2){pos.x + GetRandomValue(-5, 5), pos.y}, {(float)GetRandomValue(-30, 30), (float)GetRandomValue(-100, -200)}, WHITE, 0.3f, true}; } }
void UpdateParticles(float dt) { for (int i = 0; i < MAX_PARTICLES; i++) { if (!particles[i].active) continue; particles[i].life -= dt; if (particles[i].life <= 0) { particles[i].active = false; continue; } particles[i].position.x += particles[i].velocity.x * dt; particles[i].position.y += particles[i].velocity.y * dt; particles[i].velocity.y += 400 * dt; } }
void DrawParticles() { BeginBlendMode(BLEND_ADDITIVE); for (int i = 0; i < MAX_PARTICLES; i++) if (!particles[i].active) continue; else DrawRectangleV(particles[i].position, (Vector2){4, 4}, Fade(particles[i].color, particles[i].life)); EndBlendMode(); }

// --- CARREGAMENTO DE GIF (CORRIGIDO E ROBUSTO) ---
static void LoadGifCorrect(const char *path) {
    if (animFrames != NULL) { for (int i = 0; i < animFramesCount; i++) UnloadTexture(animFrames[i]); free(animFrames); animFrames = NULL; }
    if (gifData.data != NULL) UnloadImage(gifData); gifData = (Image){0};
    if (staticSprite.id > 0) UnloadTexture(staticSprite); staticSprite = (Texture2D){0};

    animFramesCount = 0; hasAnim = false; hasStatic = false;
    int frames = 0;

    gifData = LoadImageAnim(path, &frames);

    if (gifData.data == NULL || frames <= 0) {
        printf("AVISO GIF: LoadImageAnim falhou. Frames: %d. Tentando Fallback estatico...\n", frames);
        staticSprite = LoadTexture(path);
        hasStatic = (staticSprite.id > 0);
        return;
    }

    if (gifData.format != PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) ImageFormat(&gifData, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8);
    ...
    for (int i = 0; i < frames; i++) {
        Image frameImg = {
            .data = (unsigned char*)gifData.data + (i * frameSize),
            .width = gifData.width,
    ...
    printf("GIF: '%s' carregado com %d frames.\n", path, frames);
}

static void UnloadGifCorrect(void) {
    if (hasAnim) { for (int i = 0; i < animFramesCount; i++) UnloadTexture(animFrames[i]); free(animFrames); animFrames = NULL; hasAnim = false; }
    if (gifData.data != NULL) UnloadImage(gifData); gifData = (Image){0};
    if (hasStatic) UnloadTexture(staticSprite); staticSprite = (Texture2D){0}; hasStatic = false;
}
...
bool GuitarHero_Init(int width, int height) {
    ...
    // TENTA CARREGAR GIF (CORREÇÃO DE FORMATO)
...
void GuitarHero_UpdateDraw(float dt) {
    ...
    if (ghState == STATE_START) {
        ...
        if (hasAnim) { animTimer += dt; if (animTimer >= ANIM_FRAME_SECONDS) { animTimer = 0.0f; animCurrentFrame = (animCurrentFrame + 1) % animFramesCount; } }
        ...
        for (int i = 0; i < noteCount; i++) {
            if (!notes[i].active || !notes[i].hit) continue; if (notes[i].sustain > 0) {
                if (currentTime < notes[i].time + notes[i].sustain) { if (IsKeyDown(key_bindings[notes[i].fret])) { score += 100 * dt; if (GetRandomValue(0,5)==0) SpawnSustainSparks((Vector2){fret_positions[notes[i].fret], current_hit_zone_y}, fret_colors[notes[i].fret]); } else { notes[i].active = false; combo = 0; } }
            }
        }
        for (int fret = 0; fret < NUM_FRETS; fret++) {
            if (IsKeyPressed(key_bindings[fret])) {
                ...
                if (bestIdx != -1) { notes[bestIdx].hit = true; score += 100 + (combo * 10); combo++; SpawnExplosion((Vector2){fret_positions[fret], current_hit_zone_y}, fret_colors[fret]); if (notes[bestIdx].sustain < 0.1f) notes[bestIdx].active = false; health += 5.0f; if(health>100) health=100; }
                else { combo = 0; score -= 50; if(score<0) score=0; SpawnMiss((Vector2){fret_positions[fret], current_hit_zone_y}); fret_miss_timer[fret] = 0.3f; health -= 5.0f; if(health<=0) { health=0; ghState = STATE_LOSE; } }
            }
        }
        UpdateParticles(dt);
    }
    ...
    // Laterais
    DrawRectangle(0, 0, GUTTER_WIDTH, h, Fade(BLACK, 0.4f)); DrawRectangleLines(0, 0, GUTTER_WIDTH, h, Fade(CYBER_BLUE, 0.3f));
    DrawRectangle(w - GUTTER_WIDTH, 0, GUTTER_WIDTH, h, Fade(BLACK, 0.4f)); DrawRectangleLines(w - GUTTER_WIDTH, 0, GUTTER_WIDTH, h, Fade(CYBER_BLUE, 0.3f));

    // Personagem (GIF)
    if (hasAnim) {
        Texture2D tex = animFrames[animCurrentFrame]; float scale = SPRITE_SIZE / (float)tex.width;
        // Desenha Esquerda
        DrawTextureEx(tex, (Vector2){(GUTTER_WIDTH-SPRITE_SIZE)/2, (h-SPRITE_SIZE)/2}, 0, scale, WHITE);
        // Desenha Direita (espelhado)
        Rectangle src = {0, 0, tex.width, tex.height}; Rectangle dst = {w - GUTTER_WIDTH + (GUTTER_WIDTH-SPRITE_SIZE)/2, (h-SPRITE_SIZE)/2, SPRITE_SIZE, SPRITE_SIZE};
        src.width = -src.width; DrawTexturePro(tex, src, dst, (Vector2){0,0}, 0, WHITE);
    } else if (hasStatic) {
        // FALLBACK: Imagem estática (se LoadTexture funcionou)
        float scale = SPRITE_SIZE / (float)staticSprite.width;
        DrawTextureEx(staticSprite, (Vector2){(GUTTER_WIDTH-SPRITE_SIZE)/2, (h-SPRITE_SIZE)/2}, 0, scale, WHITE);
        DrawTextureEx(staticSprite, (Vector2){w - GUTTER_WIDTH + (GUTTER_WIDTH-SPRITE_SIZE)/2, (h-SPRITE_SIZE)/2}, 0, scale, WHITE);
        DrawText("FALLBACK", (GUTTER_WIDTH-SPRITE_SIZE)/2 + 40, (h-SPRITE_SIZE)/2 + 10, 20, YELLOW);
    }
    else {
        // DEBUG VISUAL: QUADRADO VERMELHO (Falha Total)
    ...
    for(int i=0; i<NUM_FRETS; i++) { float x = fret_positions[i]; Color c = fret_colors[i]; bool pressed = (ghState==STATE_PLAYING) && IsKeyDown(key_bindings[i]); if(pressed) DrawRectangleGradientV(x-50+2, 0, 96, current_hit_zone_y, Fade(c,0), Fade(c,0.3f)); DrawCircle(x, current_hit_zone_y, 25, Fade(c, pressed?1.0f:0.3f)); DrawCircleLines(x, current_hit_zone_y, 28, WHITE); if(fret_miss_timer[i]>0) DrawText("X", x-10, current_hit_zone_y-15, 30, RED); }
    float currentTime = haveSong ? GetMusicTimePlayed(song) + audioOffset : 0.0f;
    for(int i=0; i<noteCount; i++) { if(!notes[i].active) continue; float dy = (notes[i].time - currentTime) * SPEED; float y = current_hit_zone_y - dy; float x = fret_positions[notes[i].fret]; Color c = fret_colors[notes[i].fret]; if(notes[i].sustain > 0) { float ty = current_hit_zone_y - ((notes[i].time + notes[i].sustain - currentTime) * SPEED); float hy = notes[i].hit ? current_hit_zone_y : y; if(hy > ty) DrawRectangle(x-5, ty, 10, hy-ty, Fade(c, 0.6f)); } if(!notes[i].hit && y < h+50 && y > -50) { DrawCircle(x, y, 20, c); DrawCircle(x, y, 12, WHITE); } }

    DrawParticles(); DrawVignette(w, h); DrawScanlines(w, h);
